...
async function initMap(){
  ...
  const map = L.map(el, {scrollWheelZoom:false}).setView([25.05, 55.2], 8);
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
    attribution:'&copy; OpenStreetMap &copy; CARTO', maxZoom:19
  }).addTo(map);

  const icon = L.divIcon({
    className:'jj-pin', iconSize:[30,30], iconAnchor:[15,15], popupAnchor:[0,-14],
    html:'<span>JJ</span>'
  });

  const group = [];
  pinned.forEach(b => {
    const m = L.marker([b.lat, b.lng], {icon, title:`JJ Chicken — ${b.name}`}).addTo(map);
    m.bindPopup(
  ...
  if(group.length) map.fitBounds(group, {padding:[44,44]});
}
